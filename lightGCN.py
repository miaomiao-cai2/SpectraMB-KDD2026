...
class LightGCN(nn.Module):
    ...
    def __init__(self, device, n_layers, n_users, n_items, interaction_matrix,embedding_size):
        ...
        # self.item_embedding = nn.Embedding(self.item_count, embedding_size, padding_idx=0)
    ...
    def _get_a_adj_matrix(self):
        ...
        A_sparse = torch.sparse_coo_tensor(index, data, A_adj.shape)
        return A_sparse

    def forward(self, user_id_preference,item_id_preference):


        in_embs = torch.cat((user_id_preference, item_id_preference), dim=0)
        result = [in_embs]
